...
const validateEnv = () => {
  ...
  if (process.env.NODE_ENV === 'production') {
    requiredInProduction.forEach((key) => {
      if (!process.env[key] || process.env[key].trim() === '') {
        errors.push(`Missing required variable (production): ${key}`);
      }
    });
  }
  ...
  // ─── 7. Validate EMAIL_PORT if provided ──────────────
  if (process.env.EMAIL_PORT) {
    const emailPort = parseInt(process.env.EMAIL_PORT, 10);
    if (isNaN(emailPort) || emailPort < 1 || emailPort > 65535) {
      errors.push(`Invalid EMAIL_PORT: "${process.env.EMAIL_PORT}". Must be a number between 1 and 65535.`);
    }
  }
